...
import { useCreateBrandMutation, useGetSingleBrandInfoQuery, useUpdateBrandMutation } from "../../Feature/API/brandApi";
...
const BrandEditing = () => {
  ...
  const existedData=useGetSingleBrandInfoQuery({id,token});
  console.log(existedData?.data?.data?.name);
  ...
  useEffect(()=>{
    setBrandData({
        name: existedData?.data?.data?.name,
        company: existedData?.data?.data?.company,
        information: existedData?.data?.data?.information,
        photo: existedData?.data?.data?.photo,
        agent: existedData?.data?.data?.agent,
        phone: existedData?.data?.data?.phone,
      })
    },[existedData])
    ...
    <div>
      ...
      <section className={`flex flex-col gap-5 bg-[#161618] p-10 w-full`}>
       ...
       <div
        ...
        >
          <Button icon={true} text={"Edit Brand"} />
        </div>
      </section>
    </div>
  );
